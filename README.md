Digital-Literacy-Assignment
Student Details
Name: Deepak Gupta

Registration Number: 25BAI10811

Branch: Computer Science Engineering (Ai and Ml)

Project Overview
This repository includes all deliverables for the Student Digital Ambassador project. The goal was to build digital literacy, create a professional online presence, practice collaboration, and understand cyber safety and etiquette. The work is divided into five modules, each focusing on a practical skill from visual communication to cybercrime awareness.

Task Summaries and Reports
Task 1: Digital Literacy Awareness Infographic
Summary: I created an infographic to help classmates understand digital literacy basics. It covers useful tools, safe internet habits, and how to maintain a professional online presence.

Folder Link: [task-1-presentation/](https://github.com/d3ep4k2022-spec/Digital-Literacy./tree/main/task-1-presentation)

Project Report: I used Canva to design the infographic for its intuitive templates and structured layouts. The content was organized into three core pillars:

Essential digital tools for CSE students.

Building a professional online presence.

Foundational email etiquette.
I chose a clean, professional color scheme to prioritize readability. The primary challenge was condensing broad topics like "safe internet practices" into concise, actionable points. This task significantly improved my ability to present technical information visually.

Task 2: Build Your Student Digital Portfolio
Summary: I built a professional digital portfolio across key platforms to support academic and career growth.

Folder Link: https://github.com/d3ep4k2022-spec/Digital-Literacy./tree/main/task-2-portfolio

Project Report: I established profiles on GitHub, LinkedIn, and HackerRank.

GitHub serves as a repository for my C++ projects, Linux configurations, and group assignments.

LinkedIn acts as my professional resume to track achievements and network.

HackerRank showcases my algorithmic problem-solving skills.
Throughout my four years at VIT Bhopal, I will use GitHub to document my version-controlled progress, HackerRank for technical interview prep, and LinkedIn to reflect my certifications and event participations.

Task 3: Explore Coding and Collaboration Platforms
Summary: I practiced coding through beginner challenges and created a collaborative quiz using Google Forms.

Folder Link: 
Google Form Link: Digital Literacy Quiz

Project Report: I completed beginner C++ challenges on HackerRank, which allowed me to test logic and syntax efficiently. Additionally, I developed a Digital Literacy Awareness Quiz using Google Forms to gauge peer understanding. These tools are vital for my CDS specialization; HackerRank strengthens my foundation in Discrete Mathematics, while Google Workspace tools streamline data collection and group project management.

Task 4: Draft a Professional Email and Etiquette Guide
Summary: I wrote sample professional emails and created a checklist for responsible online behavior.

Folder Link: task-4-email-etiquette/

Project Report: Poor communication can lead to missed academic and professional opportunities. For instance, a student requesting an assignment extension with an informal tone or vague subject line often faces delays or rejection. By utilizing clear subject lines and a formal structure, I’ve learned how to demonstrate respect for the recipient’s time and improve overall response rates.

Task 5: Cybercrime Awareness Case Study and Prevention Guide
Summary: I researched a prevalent cybercrime scenario and created a safety checklist specifically for the Indian student community.

Folder Link: task-5-cybercrime/

Project Report: I conducted a case study on UPI/Online Payment Fraud, specifically the "QR Code Scam."

The Insight: Most cybercrimes today rely on social engineering—creating urgency and confusion—rather than sophisticated technical hacking. Victims are often tricked into entering their UPI PIN under the false premise of "receiving" money.

Personal Reflection: What surprised me most was how easily human behavior can be exploited during a transaction. Many people believe they are safe because they haven't shared a password, not realizing that a PIN entry is a direct authorization of a "push" transaction.

Habit Change: I will now strictly follow the "No PIN for Receiving" rule and have implemented a monthly App Permission Audit on my devices to ensure no unnecessary background data tracking is occurring.

Reporting Channels: * National Cyber Crime Portal: [suspicious link removed]

Helpline: 1930
